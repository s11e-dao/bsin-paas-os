...
public class BsinRedisProvider {
     ...
     * @return 对象列表
     */
    public static <T> List<T> getCacheObjectByPrefix(String prefix) {
        RKeys rKeys = CLIENT.getKeys();
        Iterable<String> keys = rKeys.getKeysByPattern(prefix + "*");
        RBatch batch = CLIENT.createBatch();
        List<RFuture<T>> buckets = StreamSupport.stream(keys.spliterator(), false)
                .map(key -> {
                    RBucketAsync<T> bucket = batch.getBucket(key);
