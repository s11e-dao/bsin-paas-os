 ...
 */
public class SummaryEmbeddingVectorStoreMemory implements ChatMemory {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(SummaryEmbeddingVectorStoreMemory.class);
  ...
  private SummaryEmbeddingVectorStoreMemory(Builder builder) {
  ...
  public void add(ChatMessage message) {
    ...
    // vector summary cache add
    List<ChatMessage> cacheMessages = cacheMessages();
    cacheMessages.add(message);
  }

  public List<ChatMessage> triggerSummaryMessages() {
    List<ChatMessage> cacheMessages = cacheMessages();
    return summaryConversation(cacheMessages, triggerSummaryMessages, triggerSummaryTokens);
  }
  ...
  public String cacheToString() {
    List<ChatMessage> messages = cacheMessages();
    StringBuilder result = new StringBuilder();
    for (ChatMessage message : messages) {
      result.append(message.type().toString()).append(":").append(message.text()).append("\n");
    }
    if (messages.size() == 0) {
      return null;
    }
  ...
  public List<ChatMessage> cacheMessages() {
    return ((InVectorDataBaseEmbeddingStore) store).getCacheMessages(id);
  }
  ...
  private List<ChatMessage> summaryConversation(
      List<ChatMessage> messages, int triggerSummaryMessages, int triggerSummaryTokens) {
    if (messages.size() > triggerSummaryMessages) {
      // TODO Summary Prompt
  ...
  public void cacheClear() {
    ((InVectorDataBaseEmbeddingStore) store).deleteCacheMessages(id);
  }
  ...
  public static class Builder {
    ...
    public SummaryEmbeddingVectorStoreMemory build() {
      return new SummaryEmbeddingVectorStoreMemory(this);
    }
  }

  public static SummaryEmbeddingVectorStoreMemory withMaxMessages(int maxMessages) {
    return builder().maxMessages(maxMessages).build();
  }

  public static SummaryEmbeddingVectorStoreMemory withTriggerSummaryMessages(
      int triggerSummaryMessages) {
    return builder().triggerSummaryMessages(triggerSummaryMessages).build();
  }

  public static SummaryEmbeddingVectorStoreMemory withTriggerSummaryTokens(
      int triggerSummaryTokens) {
