...
func NewUserService(cache *cache.Cache, user *openwechat.User, bot *openwechat.User) UserServiceInterface {
	return &UserService{
		cache: cache,
		user:  user,
		bot:   bot,
...
func (s *UserService) SetUserSessionContext(question, reply string) {
	value := question + "\n" + reply
	s.cache.Set(s.user.ID(), value, time.Second*config.WechatSetting.SessionTimeout)
}
