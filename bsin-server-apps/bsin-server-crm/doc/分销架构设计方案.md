...
| v2.1 | 2024-01-01 | 完善版本，集成现有`crm_grade`等级体系<br/>- 利用现有等级管理体系，统一多角色等级管理<br/>- 创建客户、代理商等级关系表<br/>- 实现更完善的等级升级机制和特权管理 | AI助手 |
...
### ✅ 主要改进
1. **去除冗余设计**：不再在客户表和代理商表中添加简单的等级字段
...
- crm_sys_agent表:
  - agent_no: "A002"
  - agent_level: "DISTRICT" (代理商类型:区级代理商)
  - region_level: 4
...
- crm_sys_agent表:
  - agent_no: "A003"
  - agent_level: "COUNTY" (代理商类型:县级代理商)
  - region_level: 3
  - county_code: "1101"
  - total_sub_agents: 5
...
3. **多级分佣计算**
   ...
   - 更新各级代理商的total_commission和current_performance
...
2. 多级代理商推广场景涉及表:
- crm_sys_agent(代理商信息,包含层级和等级信息)
- crm_agent_hierarchy(代理商层级关系)
- crm_merchant(商户信息)
...
3. 代理商层级体系:
- 省级代理商(AGENT_PROVINCE类型): 管理市级代理商 - 青铜/白银/黄金/钻石四个等级
- 市级代理商(AGENT_CITY类型): 管理县级代理商 - 青铜/白银/黄金/钻石四个等级  
- 县级代理商(AGENT_COUNTY类型): 管理区级代理商 - 青铜/白银/黄金/钻石四个等级
- 区级代理商(AGENT_DISTRICT类型): 管理镇级代理商 - 青铜/白银/黄金/钻石四个等级
- 镇级代理商(AGENT_TOWN类型): 直接管理商户 - 青铜/白银/黄金/钻石四个等级
```

### 代理商层级体系架构图

```
                    省级代理商 (AGENT_PROVINCE 类型)
                           | 
               ...
               青铜(2.0%) 白银(2.2%) 黄金(2.4%) 钻石(2.6%)
                    ...
                    ↓ 管理下级代理商
                           |
                    县级代理商 (AGENT_COUNTY 类型)
                           |
               青铜(3.0%) 白银(3.2%) 黄金(3.4%) 钻石(3.6%)
                           |
                    ↓ 管理下级代理商
                           |
                    区级代理商 (AGENT_DISTRICT 类型)
                           |
               青铜(4.0%) 白银(4.2%) 黄金(4.4%) 钻石(4.6%)
                           |
                    ↓ 管理下级代理商
                           |
                    镇级代理商 (AGENT_TOWN 类型)
                           |
               青铜(5.0%) 白银(5.2%) 黄金(5.4%) 钻石(5.6%)
                           |
                    ↓ 直接管理商户
                           |
                         商户
...
#### 🎯 核心概念区分
1. **代理商类型(agent_level)**：决定代理商的管辖范围和职责
   - AGENT_PROVINCE：省级代理商类型，管辖省级区域
   - AGENT_CITY：市级代理商类型，管辖市级区域
   - AGENT_COUNTY：县级代理商类型，管辖县级区域
   - AGENT_DISTRICT：区级代理商类型，管辖区级区域
   - AGENT_TOWN：镇级代理商类型，管辖镇级区域

2. **代理商等级(grade_no)**：在同类型代理商中的等级，影响分佣比例和权限
   - 青铜等级(L1)：新申请代理商的起始等级
   - 白银等级(L2)：达到一定业绩后的升级等级
   - 黄金等级(L3)：业绩优秀的代理商等级
   - 钻石等级(L4)：顶级代理商等级，享受最高分佣和特权

#### 📊 等级升级条件示例

| 代理商类型 | 等级 | 升级条件 | 分佣比例 | 特权 |
|----------|------|----------|----------|------|
| 省级代理商 | 青铜 | 新申请默认 | 1.0% | 基础管理权限 |
| 省级代理商 | 白银 | 管理5个下级代理或业绩50万 | 1.2% | 区域推广支持 |
| 省级代理商 | 黄金 | 管理10个下级代理或业绩100万 | 1.4% | 政策制定权 |
| 省级代理商 | 钻石 | 管理20个下级代理或业绩200万 | 1.6% | 独家资源配置 |
| 镇级代理商 | 青铜 | 新申请默认 | 5.0% | 基础商户管理 |
| 镇级代理商 | 白银 | 管理10家商户或业绩3万 | 5.2% | 商户审核权 |
| 镇级代理商 | 黄金 | 管理25家商户或业绩8万 | 5.4% | 本地推广权 |
| 镇级代理商 | 钻石 | 管理50家商户或业绩20万 | 5.6% | 优先服务权 |
...
**横向发展（类型提升）**：
```
镇级代理商 → 区级代理商 → 县级代理商 → 市级代理商 → 省级代理商
```

**综合发展路径**：
一个代理商可以同时进行纵向和横向发展，比如：
```
青铜镇代理 → 黄金镇代理 → 青铜区代理 → 白银区代理 → 青铜县代理...
```

#### 💡 设计优势
1. **清晰的职业发展路径**：代理商可以通过努力实现等级和类型的双重提升
2. **激励机制完善**：不同等级享受不同的分佣比例和特权
3. **管理层级分明**：每种类型的代理商都有明确的管辖范围
...
- **推荐人**：同事李女士（中级分销员）
- **商户**："老北京炸酱面馆"（镇级代理商管理）
- **镇级代理商**：王经理（黄金镇代理，TOWN_AGENT_L3）
- **区级代理商**：刘总（白银区代理，DISTRICT_AGENT_L2）
...
- **用户**：小区业主陈阿姨
- **推荐人**：邻居王阿姨（高级分销员）
- **商户**："品质家政服务中心"（区级代理商审核认证）
- **区级代理商**：李总（钻石区代理，DISTRICT_AGENT_L4）
- **县级代理商**：张总（黄金县代理，COUNTY_AGENT_L3）
...
- **用户群体**：大学生聚会团体（8人）
- **组织者**：学生会主席小明（初级分销员）
- **商户**："欢唱KTV南山店"（市级代理商整合资源）
- **市级代理商**：赵总（黄金市代理，CITY_AGENT_L3）
...
- **用户**：上班族刘女士
- **推荐人**：微信群活跃用户老张（金牌分销员）
- **商户**："新鲜生活超市"（县级代理商供应链整合）
- **县级代理商**：吴总（钻石县代理，COUNTY_AGENT_L4）
