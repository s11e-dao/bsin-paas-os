...
import me.flyray.bsin.mqtt.domain.channal.Chan;
...
public class MessageSenderServiceImpl implements IMessageSenderService {
  ...
  public <T> T publishWithReply(
      Class<T> clazz, String topic, CommonTopicResponse response, int retryTime) {
    ...
    while (time.getAndIncrement() <= retryTime) {
      ...
      if (Objects.nonNull(receiver)
          && receiver.getTid().equals(response.getTid())
  ...
  @Override
  public <T> ServiceReply<T> publishServicesTopic(
