...
public class JwtPlugin extends AbstractShenyuPlugin {
    ...
    protected Mono<Void> doExecute(final ServerWebExchange exchange, final ShenyuPluginChain chain, final SelectorData selector, final RuleData rule) {
        ...
        try{
            jwtBody = checkAuthorization(finalAuthorization, jwtConfig.getSecretKey());
        }catch (ExpiredJwtException e){
            Object error = ShenyuResultWrap.error(exchange, 403,"Token expired",null);
            return WebFluxResultUtils.result(exchange, error);
        }catch (Exception e){
            jwtBody = null;
        }
    ...
    private Map<String, Object> checkAuthorization(final String authorization,
        ...
        if (jwtParser.isSigned(authorization)) {
            ...
            Jwt jwt = jwtParserExec.parse(authorization);
            if (jwt == null) {
                return null;
            }
