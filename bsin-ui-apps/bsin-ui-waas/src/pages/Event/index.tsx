...
import {
  ...
  editPageList,
  deletePageList,
  getEventModelPageList,
  addEventModelPageList,
  deleteEventModelPageList,
  getRuleModelList,
  getInferenceModelList,
  getFlowModelList,
  getFormModelList,
} from './service';
...
const DictManagement = () => {
  ...
  // 控制查看的字典的 DictType
  const [showEventModel, setShowEventModel] = React.useState<DictColumnsItem>();
  // 控制查看中新增、编辑模态框
  const [isCheckAddModalVisible, setIsCheckAddModalVisible] = React.useState(
    false,
  );
  // 控制查看中新增、编辑模态框title
  ...
  // 查看字典新增
  const EventModelAdd = () => {
    console.log('EventModelAdd');
    checkForm.setFieldsValue({ dictType: showEventModel?.dictType });
  ...
  const handleCheckAddModalOk = () => {
    ...
    checkForm.validateFields().then(async () => {
      ...
      if (checkAddModalTitle === '新增') {
        let res = await addEventModelPageList({
          ...response,
          dictId: showEventModel?.id,
        });
        console.log(res);
        res ? message.success('新增成功') : message.error('添加失败！');
      }
  ...
  const CheckConfirmDel = async (id?: string | number) => {
    ...
    let res = await deleteEventModelPageList({ id });
    console.log(res);
  ...
  // 定义字典项查看操作单元格
  const CheckColumnsOptionRender = (text: any, record: EventModelColumnsItem) => {
    return [
      <div key={record.id}>
  ...
  const EventModelRef = React.useRef<ActionType>();
  // 字典新增、编辑表单
  ...
  const getModelList = async (value) => {
    console.log(value);
    // 根据类型查询对应的模型列表
    let res = null
    if (value === 1) {
    ...
    <>
      ...
      >
        <ProTable<DictItemColumnsItem>
          actionRef={EventModelRef}
          scroll={{ x: 900 }}
          columns={DictItemColumnsData}
          params={{ dictType: showEventModel?.dictType }}
          // 请求的数据
          request={async (params) => {
            console.log(params);

            let res = await getEventModelPageList({
              ...params,
